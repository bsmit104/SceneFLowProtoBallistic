class Gameplay extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("pacmap", "pacmap.png", {
      frameWidth: 16,
      frameHeight: 16,
    });
  }
  ...
  create() {
    ...
    for (let i = 0; i < fishPositions.length; i++) {
      ...
      const redfish = this.physics.add
        .sprite(position.x, position.y, "fish")
        .setScale(3)
        .setDepth(2);
    ...
    const changeDirectionIntervals = [
      1000, 1500, 2000, 2500, 500, 1000, 100, 1000,
    ];

    for (let i = 0; i < spritePositions.length; i++) {
      const position = spritePositions[i];
      const escapingSprite = this.physics.add
        .sprite(position.x, position.y, "peng")
        .setScale(3)
        .setDepth(2);
